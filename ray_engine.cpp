...
unsigned int Ray_engine::z_buffer[640] ; // to DO, change 640
...
Ray_engine::Ray_engine() {
	sprite_array[0].set_type(0);
	sprite_array[0].set_x(5);
	sprite_array[0].set_y(1);
}
...
void Ray_engine::render(SDL_Renderer* renderer, Player* my_player){
	...
	for(int w = 0 ; w < width ; w++) {	
		...
		int wall_height = (int) ((1.0*wall_max_height*__RAY_ENGINE_WALL_HEIGHT__)/ray_length) ; //wall_height >>=1 ;
	...
	for(int i=0;i<NB_SPRITE_MAX;i++) {
		float sprite_vector_x = sprite_array[i].get_x() - x ; 
		float sprite_vector_y = sprite_array[i].get_y() - y ; 
		
		float plan_vector_x = const_trig::fast_cos(player_angle) ; 
		float plan_vector_y = const_trig::fast_sin(player_angle) ; 		
		float dist_with_sprite = sqrt(sprite_vector_x*sprite_vector_x+sprite_vector_y*sprite_vector_y)+0.000000001; // +0.0000001 to avoid division by 0 ;
		int sin_current_angle_with_sprite = int(((sprite_vector_y*plan_vector_x-sprite_vector_x*plan_vector_y)/dist_with_sprite)*half_width)+half_width ; 
		//int sin_current_angle_with_sprite = int(sqrt(1-tmp_current_angle_with_sprite*tmp_current_angle_with_sprite)*320+320) ; 
		
		if (sin_current_angle_with_sprite >= 0) {			
			//std::cerr << std::setprecision(5)  << sin_current_angle_with_sprite << std::endl;
			Source = {0, 0, 3, 240} ;
			Dest = {sin_current_angle_with_sprite, 240, 3, int(240.0/dist_with_sprite)} ;
			SDL_RenderCopy(renderer, sprite_texture[0], &Source, &Dest) ;
		}
		
		
		
		std::cerr << std::setprecision(5)  << " " << (sprite_vector_y*plan_vector_x-sprite_vector_x*plan_vector_y)/dist_with_sprite << " " << const_trig::fast_sin(player_angle) << "  current_angle : " << player_angle << std::endl;
	}
